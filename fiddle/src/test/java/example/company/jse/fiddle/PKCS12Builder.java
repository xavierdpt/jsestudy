package example.company.jse.fiddle;

import example.company.asn.AsnEncoding;
import example.company.asn.elements.Asn;
import example.company.asn.utils.OIDS;
import example.company.tox.common.Common;

public class PKCS12Builder {

	private byte[] hash = Common.bytes("B72974C662F5B884D336678666D7D9F7D5877464");
	private byte[] something = Common.bytes("6CB867698D2BF58AC0ECA315C192DEC3C76AF213");
	private byte[] localKeyId = Common.bytes("54696D652031353237323430363739303233");
	private byte[] friendlyNames = Common.bytes("0063006C00690065006E007400630061");
	private byte[] pbeSha3DESCBCs = Common.bytes("8FD2439C4F78EB6612D94F642C7B75FBBF5FF371");
	private byte[] something2 = Common.bytes(
			"");
	private byte[] something3s = Common.bytes("1B9DCA53442A0222F5DA254DBBC62C7C6FD2E60E");
	private byte[] something4s = Common.bytes(
			"");

	public byte[] build() {

		return Asn.seq(

				Asn.integer(3),

				Asn.seq(

						Asn.oid(OIDS.DATA),

						Asn.cs(0,

								Asn.os(Asn.seq(

										Asn.seq(Asn.oid(OIDS.DATA), Asn.cs(0, Asn.os(

												Asn.seq(

														Asn.seq(

																Asn.oid(OIDS.PKCS_8_SHROUDED_KEY_BAG),

																Asn.cs(0, Asn.seq(

																		Asn.seq(

																				Asn.oid(OIDS.PBE_WITH_SHA_AND_3_KEY_TRIPLE_DES_CBC),

																				Asn.seq(

																						Asn.os(pbeSha3DESCBCs),

																						Asn.integer(50000)

																				)

																		),

																		Asn.os(something2)

																)),

																Asn.set(

																		Asn.seq(

																				Asn.oid(OIDS.FRIENDLY_NAME),

																				Asn.set(

																						Asn.bmpstring(friendlyNames)

																				)

																		),

																		Asn.seq(

																				Asn.oid(OIDS.LOCAL_KEY_ID),

																				Asn.set(Asn.os(localKeyId))

																		)

																)

														))

										))),

										Asn.seq(Asn.oid(OIDS.ENCRYPTED_DATA), Asn.cs(0,

												Asn.seq(

														Asn.integer(0),

														Asn.seq(

																Asn.oid(OIDS.DATA),

																Asn.seq(

																		Asn.oid(OIDS.PBE_WITH_SHA_AND_40_BIT_RC2_CBC),

																		Asn.seq(

																				Asn.os(something3s),

																				Asn.integer(50000)

																		)

																),

																Asn.cs(0, something4s, AsnEncoding.PRIMITIVE)

														)

												)

										))

								))

						)

				),

				Asn.seq(

						Asn.seq(

								Asn.seq(

										Asn.oid(OIDS.HASH_ALGORITHM_IDENTIFIER),

										Asn.n()

								),

								Asn.os(hash)

						),

						Asn.os(something),

						Asn.integer(100000)

				)

		).encode();

	}
}
